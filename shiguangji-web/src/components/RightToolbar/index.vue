        ...
        <el-dropdown trigger="click" :hide-on-click="false" style="padding-left: 12px" v-if="showColumnsType == 'checkbox'">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              ...
              <template v-for="(item, key) in columnList" :key="key">
                <el-dropdown-item>
                  <el-checkbox v-model="item.visible" @change="checkboxChange($event, String(key))" :label="item.label" />
                </el-dropdown-item>
              </template>
            </el-dropdown-menu>
          </template>
...
const props = defineProps({
  ...
  /* 显隐列信息（数组格式、对象格式） */
  columns: {
    type: [Array, Object] as PropType<TableShowColumns[] | Record<string, TableShowColumns>>,
    default: () => ({})
  },
  /* 是否显示检索图标 */
  search: {
...
// 统一成数组供模板遍历（columns 支持数组/对象两种格式）
const columnList = computed(() => Array.isArray(props.columns) ? props.columns : Object.values(props.columns))
...
const transferData = computed(() => { const cols = props.columns; return Array.isArray(cols) ? cols.map((item, index) => ({ key: index, label: item.label })) : Object.keys(cols).map((key, index) => ({ key: index, label: cols[key].label })) })
...
function dataChange(data: number[]): void {
  ...
  if (Array.isArray(cols)) {
    cols.forEach((col, index) => {
      col.visible = !data.includes(index)
    })
  } else {
...
if (props.storageKey) {
  try {
    ...
    if (saved && typeof saved === 'object') {
      const cols = props.columns
      if (Array.isArray(cols)) {
        cols.forEach((col: TableShowColumns, index: number) => {
          if (saved[index] !== undefined) col.visible = saved[index]
        })
      } else {
        Object.keys(cols).forEach(key => {
          if (saved[key] !== undefined) cols[key].visible = saved[key]
        })
      }
    }
  } catch (e) {}
...
function saveStorage(): void {
  ...
  try {
    ...
    const cols = props.columns
    if (Array.isArray(cols)) {
      cols.forEach((col: TableShowColumns, index: number) => { state[index] = col.visible })
    } else {
