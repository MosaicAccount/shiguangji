...
| `hitTotal` | 出参 | 全文命中次数，供「文中出现 N 次 / 文中多次出现」小字（2–9 次显示具体次数，≥10 次统一为「多次」；仅 >1 处时显示，评审结论见 §2.9：多命中不轮播） |
...
- **多命中轮播已评审不采用**（2026-09-18）：文字的阅读节奏与自动播放冲突，动的卡片还会干扰相邻卡片的阅读；多命中场景以摘要下方的出现次数小字提示（2–9 次显示「文中出现 N 次」，≥10 次统一为「文中多次出现」——具体大数字对读者不可行动，也不用「命中」这类实现术语），查看全部进详情页。完整规格留档于 `docs/design/mockups/note-0.0.2/note-search-carousel.html`，落地样式见同目录 `note-search-excerpt.html`；
