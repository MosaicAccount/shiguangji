...
| `hitTotal` | 出参 | 全文命中次数，供「共 N 处命中」小字（仅 >1 处时显示，评审结论见 §2.9：多命中不轮播） |
...
- **多命中轮播已评审不采用**（2026-09-18）：文字的阅读节奏与自动播放冲突，动的卡片还会干扰相邻卡片的阅读；多命中场景以摘要下方的「共 N 处命中」小字提示（仅 >1 处时显示），查看全部进详情页。完整规格留档于 `docs/design/mockups/note-0.0.2/note-search-carousel.html`，落地样式见同目录 `note-search-excerpt.html`；
